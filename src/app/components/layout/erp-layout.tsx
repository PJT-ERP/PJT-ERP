...
const ROLE_NAVIGATION: Record<UserRole, NavItemDef[]> = {
  ...
  Owner: [
    ...
    { label: "Executive Dashboard", icon: <LayoutDashboard size={15} />, path: "/erp/dashboard" },
    { label: "Customer Analytics", icon: <BarChart2 size={15} />, path: "/erp/customer-analytics" },
    { label: "MONITORING", isHeader: true },
  ...
  const notifications = React.useMemo(() => {
    ...
    if (role === 'Owner') {
      ...
      quotations.filter(q => q.status === 'waiting_pricing').forEach(q => {
        notifs.push({ id: q.id, type: 'info', title: 'Menunggu Estimasi Finance', desc: `Quotation ${q.id} sedang dihitung harga pokoknya.` });
      });
      ...
      salesOrders.filter(so => so.status === 'Pending Design').forEach(so => {
        notifs.push({ id: so.id, type: 'warning', title: 'Desain Baru Dibutuhkan', desc: `SO ${so.id} menunggu kelengkapan desain & BOM.` });
      });
      if (role === 'Engineering Supervisor') {
        salesOrders.filter(so => so.status === 'Waiting Approval').forEach(so => {
          notifs.push({ id: so.id, type: 'warning', title: 'Desain Butuh Review', desc: `SO ${so.id} menunggu persetujuan (Approval) Anda.` });
        });
      }
