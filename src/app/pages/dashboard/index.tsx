...
export function DashboardPage() {
  ...
  const kpis = [
    { label: 'Total SO Aktif', value: executiveDashboard?.totalOrders ?? salesOrders.filter(s => s.status !== 'Completed').length, icon: Package, color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' },
    { label: 'Dalam Produksi', value: executiveDashboard?.inProgress ?? salesOrders.filter(s => s.status === 'In Production').length, icon: Clock, color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
    { label: 'Selesai (Total)', value: executiveDashboard?.completed ?? salesOrders.filter(s => s.status === 'Completed').length, icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-50', border: 'border-green-200' },
    { label: 'Menunggu QC', value: executiveDashboard?.waitingQC ?? salesOrders.filter(s => s.status === 'QC').length, icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200' },
    { label: 'Avg Durasi Prod.', value: `${avgDuration}h`, icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-200' },
    { label: 'SO Terlambat', value: executiveDashboard?.overdueCount ?? overdueSOs.length, icon: AlertTriangle, color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-200' },
  ];
